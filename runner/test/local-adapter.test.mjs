...
test('a dependency-graph preamble is not an issue', () => {
  assert.equal(parseIssuesFile(FILE).some((i) => i.title === 'Dependency graph'), false)
})

test('keys are stable slugs usable as branch names', () => {
...
test('an issue whose acceptance-criteria section is empty still parses, and fails the gate later', () => {
  const issues = parseIssuesFile('## Thing\n\n### Acceptance criteria\n\n### Technical notes\nnone\n')
...
test('an approved branch becomes the next issue’s base', async () => {
  ...
  try {
    ...
    await adapter.onApproved(adapter.issues[0], { branch: 'feature/auth-issue-1', head: 'abc', cycle: 1 }, {})
