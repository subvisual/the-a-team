...
const cfg = { ...DEFAULTS, maxCycles: 3, cleanupOnSuccess: false }
...
function adapterSpy(extra = {}) {
  ...
  return {
    ...
    repoPath,
    base: 'main',
    async openBlockers() { return [] },
    async onNeedsDetail(i, reason) { calls.push(['needsDetail', reason]) },
    async onClaimed() { calls.push(['claimed']) },
    async onImplemented() { calls.push(['implemented']); return {} },
    async onVerdict(i, ctx, v) { calls.push(['verdict', v.verdict, ctx.cycle]) },
    async onApproved() { calls.push(['approved']) },
    async onFailed(i, ctx, reason) { calls.push(['failed', reason]) },
    ...extra,
  }
}
...
const committingExecutor = (n = { i: 0 }) => async ({ worktree }) => {
  n.i += 1
  writeFileSync(join(worktree, `change-${n.i}.txt`), `pass ${n.i}\n`)
  git(['add', '-A'], worktree)
  git(['commit', '-m', `work ${n.i}`], worktree)
  return { ok: true, status: 'done', summary: `pass ${n.i}`, costUsd: 0 }
}
...
const reviewer = (verdicts) => {
  ...
  return async () => {
    ...
    i += 1
    return { ok: true, testsRan: true, testsPassed: true, notes: '', unmetAc: [], costUsd: 0, sessionId: 's1', ...v }
  }
}
...
test('an issue with no acceptance criteria is refused before anything is claimed', async () => {
  ...
  assert.deepEqual(adapter.calls.map((c) => c[0]), ['needsDetail'])
})

test('approval on the first cycle runs one implement and one review', async () => {
  const adapter = adapterSpy()
  const result = await runIssue({
    adapter, issue: issue(), cfg,
    deps: { execute: committingExecutor(), review: reviewer([{ verdict: 'approve' }]) },
  })
  assert.equal(result.outcome, OUTCOME.approved)
  assert.equal(result.cycles, 1)
  assert.deepEqual(adapter.calls.map((c) => c[0]), ['claimed', 'implemented', 'verdict', 'approved'])
})
...
test('the cycle cap ends the run rather than looping forever', async () => {
  const adapter = adapterSpy()
  const result = await runIssue({
    adapter, issue: issue(), cfg: { ...cfg, maxCycles: 2 },
    deps: {
      execute: committingExecutor(),
      review: reviewer([{ verdict: 'request-changes', unmetAc: [{ criterion: 'it works', why: 'still no' }] }]),
    },
  })
...
test('an executor that reports done but commits nothing fails the run', async () => {
  ...
  const result = await runIssue({
    adapter, issue: issue(), cfg,
    deps: {
      execute: async () => ({ ok: true, status: 'done', summary: 'lied', costUsd: 0 }),
...
test('a blocked executor escalates instead of guessing', async () => {
  ...
  const result = await runIssue({
    adapter, issue: issue(), cfg,
    deps: {
      execute: async () => ({ ok: false, status: 'blocked', blockedReason: 'criterion 2 contradicts the schema', costUsd: 0 }),
      review: reviewer([{ verdict: 'approve' }]),
    },
  })
  assert.equal(result.outcome, OUTCOME.failed)
  assert.match(result.reason, /contradicts the schema/)
  assert.equal(adapter.calls.some((c) => c[0] === 'verdict'), false)
})

test('open blockers skip the issue without claiming it', async () => {
  const adapter = adapterSpy({ async openBlockers() { return [4] } })
...
test('the claim is released, so a second run can take the issue', async () => {
  const adapter = adapterSpy()
  const deps = { execute: committingExecutor(), review: reviewer([{ verdict: 'approve' }]) }
